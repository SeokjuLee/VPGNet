...
int main(int argc, char** argv) {
  ...
  if (db_backend == "leveldb") {
    driving_data.ParseFromString(it->value().ToString());
  } else if (db_backend == "lmdb") {
    driving_data.ParseFromArray(mdb_value.mv_data, mdb_value.mv_size);
  } else {
    LOG(FATAL) << "Unknown db backend " << db_backend;
  }

  const Datum& datum = driving_data.car_image_datum();
  sum_blob.set_num(1);
